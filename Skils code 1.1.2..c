...
resetMotorEncoder(leftMotor);
		...
		deg = deg*PI/180*10*627/12.5;

	if(dir > 0)
	{
		while(nMotorEncoder[leftMotor] < deg)
		{
			motor[leftMotor]  =  127;
...
void claw(int time, int dir)
{
	...
	motor[claw1] = motor[claw2] = dir*20;
}
...
resetMotorEncoder(leftMotor);
while(nMotorEncoder(leftMotor) > -392)
{
motor[lift1] = motor[lift2] = 127;
motor[rightMotor] = motor[leftMotor] = -127;
}
...
while(nMotorEncoder(leftMotor) < 5*392)
{
motor[lift1] = motor[lift2] = 127;
motor[rightMotor] = motor[leftMotor] = -127;
}
...
motor[leftMotor] = motor[rightMotor] = motor[lift1] = motor[lift2] = 0;
}
...
task usercontrol()
{
	...
	while (true)
	{
		...
		else if(nMotorEncoder[lift1] > 300)
			{
